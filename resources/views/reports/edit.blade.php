...
@section('title', 'Modifica Rapportino - Rapportini')

@section('page-title', 'Modifica Rapportino')
...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0"><i class="bi bi-file-text me-2"></i>2. Compila Dettagli Rapportino</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('interventions.reports.update', [$intervention, $report]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="time" class="form-control @error('start_time') is-invalid @enderror"
                           id="start_time" name="start_time" value="{{ old('start_time', $report->start_time ? substr($report->start_time, 0, 5) : '') }}">
                    @error('start_time')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="time" class="form-control @error('end_time') is-invalid @enderror"
                           id="end_time" name="end_time" value="{{ old('end_time', $report->end_time ? substr($report->end_time, 0, 5) : '') }}">
                    @error('end_time')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('activities') is-invalid @enderror"
                          id="activities" name="activities" rows="4" placeholder="Descrivi le attività svolte...">{{ old('activities', $report->activities) }}</textarea>
                @error('activities')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('notes') is-invalid @enderror"
                          id="notes" name="notes" rows="3" placeholder="Note aggiuntive...">{{ old('notes', $report->notes) }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                @livewire('media-manager', ['mediableType' => 'App\\Models\\Report', 'mediableId' => $report->id])
            </div>
